...
namespace AstraSim {
    ...
    bool Sys::initialize_sys(string name) {
        ...
        if (j.contains("preferred-dataset-splits")) {
            this->preferred_dataset_splits = j["preferred-dataset-splits"];
        }
        if (j.contains("peak-perf")) {
            this->peak_perf = j["peak-perf"];
            this->peak_perf = this->peak_perf * 1000000000000;  // TFLOPS
        }
        if (j.contains("local-mem-bw")) {
            // DRAM bandwidth
            this->local_mem_bw = j["local-mem-bw"];
            this->local_mem_bw = this->local_mem_bw * 1000000000;  // GB/sec -> B/sec
        }
        if (j.contains("roofline-enabled")) {
            if (j["roofline-enabled"] != 0) {
                this->roofline_enabled = true;
                this->roofline = new Roofline(this->local_mem_bw, this->peak_perf);
            }
        }
        ...
        this->single_device_mode = false;
        if (j.contains("single-device-simulation")) {
            if (j["single-device-simulation"] != 0) {
                this->single_device_mode = true;
            }
            else {
                this->single_device_mode = false;
            }
        ...
        this->scalesim_enabled = false;
        this->num_arrays = 0;
        this->array_height = 0;
        this->array_width = 0;
        this->sram_size = 0;
        if (j.contains("scalesim-enabled")) {
            if (j["scalesim-enabled"] != 0) {
                this->scalesim_enabled = true;

                // Set num_arrays, array_height, array_width
                if (j.contains("num-arrays") && j["num-arrays"] > 0) {
                    this->num_arrays = j["num-arrays"];
                }
                else {
                    sys_panic("num-arrays must be specified when scalesim-enabled is on!");
                }
                if (j.contains("array-height") && j["array-height"] > 0) {
                    this->array_height = j["array-height"];
                }
                else {
                    sys_panic("array-height must be specified when scalesim-enabled is on!");
                }
                if (j.contains("array-width") && j["array-width"] > 0) {
                    this->array_width = j["array-width"];
                }
                else {
                    sys_panic("array-width must be specified when scalesim-enabled is on!");
                }

                // Update NPU performance given the array configuration
                this->peak_perf = 2 * this->num_arrays * this->array_height * this->array_width * 1000000000; // in FLOPS (1 sec = 10^9 cycle)

                // Set sram_size
                if (j.contains("sram-size") && j["sram-size"] > 0) {
                    this->sram_size = j["sram-size"]; // SRAM size in KB
                }
                else {
                    sys_panic("sram-size must be specified when scalesim-enabled is on!");
                }
            }
        }
    ...
    void Sys::proceed_to_next_vnet_baseline(StreamBaseline* stream) {
        ...
        // Print timing info for stream phase completion
        // if (stream->steps_finished > 0) {
        //     std::cout << "[TIMING] StreamPhaseCompleted: sys_id=" << id 
        //               << ", tick=" << Sys::boostedTick() 
        //               << ", stream_id=" << stream->stream_id
        //               << ", phase_completed=" << stream->steps_finished 
        //               << ", phases_remaining=" << stream->phases_to_go.size() << std::endl;
        // }
        ...
        if (stream->phases_to_go.size() == 0) {
            // Print timing info when stream completely finishes
            // std::cout << "[TIMING] StreamCompleted: sys_id=" << id 
            //           << ", tick=" << Sys::boostedTick() 
            //           << ", stream_id=" << stream->stream_id
            //           << ", total_phases_completed=" << stream->steps_finished 
            //           << ", creation_time=" << stream->creation_time << std::endl;
        ...
        // Print timing info when starting new phase
        // std::cout << "[TIMING] StreamPhaseStarted: sys_id=" << id 
        //           << ", tick=" << Sys::boostedTick() 
        //           << ", stream_id=" << stream->stream_id
        //           << ", phase_number=" << stream->steps_finished + 1
        //           << ", queue_id=" << stream->current_queue_id
        //           << ", comm_type=" << static_cast<uint64_t>(stream->current_com_type) << std::endl;
    ...
    int Sys::sim_send(Tick delay,
        ...
        void* fun_arg) {
        if (delay == 0) {
            // Return the actual delay from the network API
            int network_delay = comm_NI->sim_send(buffer, count, type, dst, tag, request, msg_handler,
                fun_arg);
            ...
            // For delayed sends, return the delay value
